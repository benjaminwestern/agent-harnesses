...
func NewProvider(emit func(contract.RuntimeEvent), cfg config.RuntimeConfig) *Provider {
	...
	allModels := append(modelcatalog.Codex(), models...)

	return &Provider{
		sessions: make(map[string]*session),
		emit:     emit,
		probe: providerprobe.New(codexBinaryPath, "--version").
			WithAuthCommand("login", "status").
			WithModels("built_in", allModels),
	}
}
...
func (p *Provider) Describe() contract.RuntimeDescriptor {
	descriptor := contract.NewRuntimeDescriptor(
		...
		contract.RuntimeCapabilities{
			...
			ImmediateProviderSession: false,
...
func (p *Provider) SendInput(
	...
	if len(request.Parts) > 0 {
		return nil, errors.New("codex provider does not support multimodal content parts")
	}
