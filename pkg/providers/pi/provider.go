...
import (
	...
	"github.com/benjaminwestern/agentic-control/pkg/providers/providerprobe"
)
...
func NewProvider(emit func(contract.RuntimeEvent), cfg config.RuntimeConfig) *Provider {
	...
	allModels := append(modelcatalog.Pi(), models...)

	return &Provider{
		sessions: make(map[string]*session),
		emit:     emit,
		probe: providerprobe.New(piBinaryPath, "--version").
			WithModels("runtime_default", allModels),
	}
}
...
func (p *Provider) Describe() contract.RuntimeDescriptor {
	descriptor := contract.NewRuntimeDescriptor(
		...
		contract.RuntimeCapabilities{
			...
			ResumeByProviderID:       false,
			AdoptExternalSessions:    false,
...
func (p *Provider) StartSession(ctx context.Context, request api.StartSessionRequest) (*contract.RuntimeSession, error) {
	...
	sess, err := p.spawnProcess(ctx, request.SessionID, resolvedCWD, request.Model, request.ModelOptions)
	if err != nil {
		return nil, err
	}
...
func (p *Provider) SendInput(ctx context.Context, request api.SendInputRequest) (*contract.RuntimeEvent, error) {
	...
	if len(request.Parts) > 0 {
		return nil, errors.New("pi provider does not support multimodal content parts")
	}
...
func (p *Provider) spawnProcess(ctx context.Context, appSessionID, cwd, model string, options api.ModelOptions) (*session, error) {
