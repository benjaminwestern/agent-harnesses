...
const pluginDir = path.dirname(pluginRealPath)
const harnessRoot = path.resolve(pluginDir, "..", "agentic-control")
const helperBinary = path.join(harnessRoot, "bin", "agent_harness")
const helperConfigPath = path.join(harnessRoot, "plugin-config.json")
...
function helperArgs() {
  try {
    const text = fs.readFileSync(helperConfigPath, "utf8")
    ...
    const helperArgs = Array.isArray(config?.helperArgs)
      ...
      : ["--runtime", "opencode", "--provenance", "native_plugin"]
    return { helperPath, helperArgs }
  } catch {
    // Fall back to the built-in default when the sidecar config is missing.
  }
  return {
    helperArgs: ["--runtime", "opencode", "--provenance", "native_plugin"],
  }
}

function helperInvocation() {
  const config = helperArgs()
  const binary = config.helperPath ?? helperBinary
...
function hasProjectLocalOverride(directory) {
  const projectPlugin = safeRealPath(
    path.resolve(directory, ".opencode", "plugins", "agentic-control.js"),
  )
  return projectPlugin !== undefined && projectPlugin !== pluginRealPath
}

async function emit(payload) {
  const spawnArgs = helperInvocation()
  if (!spawnArgs) return
